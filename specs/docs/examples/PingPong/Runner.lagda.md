...
This module provides the execution harness for the Ping-Pong protocol example, utilizing the [AVM.Interpreter](../../AVM/Interpreter.lagda.md) operational semantics implementation.
...
The interpreter instantiation requires concrete implementations of platform-specific parameters as specified by the AVM.Interpreter module signature.
...
pingBehaviorImpl =
  ...
  caseMaybe (extractMsg inp)
    (λ msg →
      -- Check if counter < maxCount to continue
      (if suc (PP.PingPongMsg.counter msg) <? suc (PP.PingPongMsg.maxCount msg) then
        -- Continue: send Pong message back to sender, with myId as the new partnerId
        (let nextMsg = PP.VPingPongMsg (PP.mkMsg Pong (suc (PP.PingPongMsg.counter msg)) myId (PP.PingPongMsg.maxCount msg))
         in IT._>>=_ (trigger (Obj (call (PP.PingPongMsg.partnerId msg) nextMsg))) λ mResult →
            ret (PP.VString "ping-continue" ∷ []))
      else
        -- Done
        ret (PP.VString "ping-done" ∷ [])))
    (ret (PP.VString "ping-decode-failed" ∷ []))
...
pongBehaviorImpl =
  ...
  caseMaybe (extractMsg inp)
    (λ msg →
      -- Check if counter < maxCount to continue
      (if suc (PP.PingPongMsg.counter msg) <? suc (PP.PingPongMsg.maxCount msg) then
        -- Continue: send Ping message back to sender, with myId as the new partnerId
        (let nextMsg = PP.VPingPongMsg (PP.mkMsg Ping (suc (PP.PingPongMsg.counter msg)) myId (PP.PingPongMsg.maxCount msg))
         in IT._>>=_ (trigger (Obj (call (PP.PingPongMsg.partnerId msg) nextMsg))) λ mResult →
            ret (PP.VString "pong-continue" ∷ []))
      else
        -- Done
        ret (PP.VString "pong-done" ∷ [])))
    (ret (PP.VString "pong-decode-failed" ∷ []))
...
showEventType (ObjectThawed oid ctrl) = "ObjectThawed(" ++ˢ showOid oid ++ˢ ", " ++ˢ ctrl ++ˢ ")"
...
The example program is specified in Main.lagda.md. However, due to the abstraction of the Object type in Main and the requirement for concrete ObjectImpl instances in this module, direct utilization of PP.pingPongExample is precluded. Consequently, we construct the test execution manually by directly invoking the behavioral functions.
