...
\begin{tikzpicture}[node distance=0.8cm,font=\small]
  ...
  \node[state,fill=lightblue,minimum width=7cm,minimum height=0.6cm] (l0) {Instr$_0$: obj + introspect + reflect + reify};
...
\textbf{Programs are ITrees} \cite{XiaEtAl-POPL-2020-InteractionTrees}
...
\begin{tikzpicture}[x=1cm,y=1cm,font=\small,scale=0.85,transform shape]
  ...
  % Edges
  \draw[edge,thick] (call) -- node[above left,font=\tiny] {Nothing} (tau1);
  \draw[resp,thick] (call) -- node[right,font=\tiny] {Just out} (ret);
  \draw[msg,thick]  (call) -- node[above right,font=\tiny] {Nothing} (write);
...
\begin{tikzpicture}[node distance=1.2cm,scale=0.9,transform shape]
  ...
  \draw[decorate,decoration={brace,mirror,amplitude=5pt},thick,darkblue]
